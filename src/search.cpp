...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if ((ss - 1)->currentMove.is_ok() && pos.move_dark((ss - 1)->currentMove))
    {
        constexpr auto nt = PvNode ? PV : NonPV;
...
Value Search::Worker::flip_search(
  Position& pos, Stack* ss, Value alpha, Value beta, bool isQsearch, Depth depth, bool cutNode) {
    ...
    auto               restPieces = pos.rest_pieces();
    int                total      = 0;
    ...
    for (auto& [piece, num] : restPieces)
    {
        ...
        accumulatorStack.pop();
        piece = pos.do_flip((ss - 1)->currentMove, piece, &dp, &tt);
        accumulatorStack.push(dp);
        ...
        pos.undo_flip((ss - 1)->currentMove, piece);
