        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            if (mainThread->tm.elapsed(threads.nodes_searched()) > totalTime)
            {
                ...
                threads.increaseDepth =
                  mainThread->ponder
                  || mainThread->tm.elapsed(threads.nodes_searched()) <= totalTime * 0.430;
        }
        ...
        if (!rootNode && pos.major_material(us) && bestValue > VALUE_MATED_IN_MAX_PLY)
        {
            ...
            else
            {
                ...
                if (!ss->inCheck && lmrDepth < 9
                    && ss->staticEval + (bestValue < ss->staticEval - 58 ? 139 : 115)
                           + 139 * lmrDepth
                         <= alpha)
                    continue;
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (ss->ply >= MAX_PLY)
        return !ss->inCheck ? evaluate(networks, pos, thisThread->optimism[us]) : VALUE_DRAW;
