...
static int
zip_entry_finalize(struct zip_t* zip, struct zip_entry_mark_t* entry_mark, const ssize_t n) {
...
static ssize_t
zip_files_move(MZ_FILE* m_pFile, mz_uint64 writen_num, mz_uint64 read_num, size_t length) {
    ssize_t      n         = 0;
...
struct zip_t* zip_openwitherror(const char* zipname, int level, char mode, int* errnum) {
    ...
    *errnum           = 0;
    ...
    case 'a' :
    case 'd' :
        if (!mz_zip_reader_init_file_v2_rpb(&(zip->archive), zipname,
...
static int _zip_entry_open(struct zip_t* zip, const char* entryname, int case_sensitive) {
  ...
  */
    ...
    if (pzip->m_pWrite(pzip->m_pIO_opaque, zip->entry.offset, extra_data, extra_size) != extra_size)
    {
