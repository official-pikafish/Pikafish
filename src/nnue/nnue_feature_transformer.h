...
class FeatureTransformer {
   ...
   private:
    ...
    void update_accumulator_incremental(const Position& pos, StateInfo* computed) const {
        assert((computed->accumulator.accumulation).computed[Perspective]);
        ...
        if constexpr (CurrentOnly)
            for (StateInfo* st = pos.state(); st != computed; st = st->previous)
                FeatureSet::append_changed_indices<Perspective>(bucket, mirror, st->dirtyPiece, removed,
                                                                added);
        else
        ...
        assert(!(next->accumulator.accumulation).computed[Perspective]);
        ...
        else
        {
            for (IndexType i = 0; i < HalfDimensions / TileHeight; ++i)
            {
                ...
                auto accTileIn = reinterpret_cast<const vec_t*>(
                  &computed->accumulator.accumulation[Perspective][i * TileHeight]);
                ...
                auto accTileOut = reinterpret_cast<vec_t*>(
                  &next->accumulator.accumulation[Perspective][i * TileHeight]);
                for (IndexType j = 0; j < NumRegs; ++j)
            ...
            for (IndexType i = 0; i < PSQTBuckets / PsqtTileHeight; ++i)
            {
                ...
                auto accTilePsqtIn = reinterpret_cast<const psqt_vec_t*>(
                  &computed->accumulator.psqtAccumulation[Perspective][i * PsqtTileHeight]);
                ...
                auto accTilePsqtOut = reinterpret_cast<psqt_vec_t*>(
                  &next->accumulator.psqtAccumulation[Perspective][i * PsqtTileHeight]);
                for (std::size_t j = 0; j < NumPsqtRegs; ++j)
