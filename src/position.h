...
class Position {
   public:
    ...
    void       undo_null_move();
    Piece
    do_flip(Move m, Piece pc, DirtyPiece* dp = nullptr, const TranspositionTable* tt = nullptr);
    void undo_flip(Move m, Piece fromPc);
    ...
    std::vector<std::pair<Piece, int>> rest_pieces() const;
...
inline std::vector<std::pair<Piece, int>> Position::rest_pieces() const {
    std::vector<std::pair<Piece, int>> pieces;
    for (PieceType pt = ROOK; pt < KING; ++pt)
    {
        Piece pc = make_piece(~sideToMove, pt);
        if (restPieces[pc])
